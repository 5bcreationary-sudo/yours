...
export default function Login() {
  ...
  // Only auto-redirect when onboarding is fully complete. A signed-in user
  // who hasn't finished onboarding gets to keep the form (so they can switch
  // accounts) and a "Continue setup" hint instead of being bounced away.
  useEffect(() => {
    if (user?.onboarding_complete) navigate("/app", { replace: true });
  }, [user, navigate]);
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center px-6">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        ...
        <p className="text-sm text-muted-foreground mb-8">Sign in to Yours</p>

        {user && !user.onboarding_complete && (
          <div className="mb-6 p-3 rounded-xl bg-secondary text-sm">
            <p className="font-medium">You're signed in as {user.email}.</p>
            <p className="text-muted-foreground mt-0.5">
              Want to{" "}
              <Link to="/onboarding" className="text-foreground font-medium underline underline-offset-2">
                continue setup
              </Link>{" "}
        ...
        <Button variant="outline" className="w-full h-11 rounded-xl mb-4 text-sm font-medium" onClick={handleGoogle}>
